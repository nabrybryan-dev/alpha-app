...
describe('AccionesRevision', () => {
  it('sin la capacidad, los botones están deshabilitados y un clic no llama a la base', () => {
    render(<AccionesRevision usuarioId="u-1" semanaInicio="2026-09-28" ordenes={[]} onOrdenCreada={() => {}} />)

    const botonDetener = screen.getByRole('button', { name: 'Detener publicación' })
  ...
  it('mientras cargan las capacidades, también quedan deshabilitados (nunca habilitados "por si acaso")', () => {
    ...
    render(<AccionesRevision usuarioId="u-1" semanaInicio="2026-09-28" ordenes={[]} onOrdenCreada={() => {}} />)
    expect(screen.getByRole('button', { name: 'Detener publicación' })).toBeDisabled()
  })
  ...
  it('con la capacidad: escribir un motivo y confirmar inserta con la idempotency_key correcta', async () => {
    ...
    render(<AccionesRevision usuarioId="u-1" semanaInicio="2026-09-28" ordenes={[]} onOrdenCreada={onOrdenCreada} />)

    fireEvent.click(screen.getByRole('button', { name: 'Detener publicación' }))
    fireEvent.change(screen.getByLabelText(/Motivo para detener la publicación/), {
  ...
  it('reportar riesgo: mismo cuadro, mismo contrato, con su propio texto', async () => {
    estadoCapacidades.puedeReportar = true
    ordenesMock.reportarRiesgo.mockResolvedValue({ ok: true, yaExistia: false, orden: {} })
    render(<AccionesRevision usuarioId="u-2" semanaInicio="2026-10-05" ordenes={[]} onOrdenCreada={() => {}} />)

    fireEvent.click(screen.getByRole('button', { name: 'Reportar riesgo' }))
    fireEvent.change(screen.getByLabelText(/Motivo del riesgo reportado/), { target: { value: 'dolor articular' } })
  ...
  it('ya detenida (llega en `ordenes`): muestra "Detenida por … a las …" en vez de un botón', () => {
    render(
      <AccionesRevision
        usuarioId="u-1"
        semanaInicio="2026-09-28"
    ...
    expect(screen.queryByRole('button', { name: 'Detener publicación' })).not.toBeInTheDocument()
    expect(screen.getByText(/Detenida por/)).toBeInTheDocument()
  ...
  it('errores de la base (RLS) se muestran con un mensaje claro en la página', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: 'Detener publicación' }))
    fireEvent.change(screen.getByLabelText(/Motivo para detener la publicación/), { target: { value: 'x' } })
    fireEvent.click(screen.getByRole('button', { name: 'Confirmar detener' }))
