-- ===========================================================================
-- 0073 · La tasa contra el plan se exporta: una función de SOLO LECTURA.
--
-- PARA QUÉ. La revisión semanal larga (decisión de Bryan del 12-sep: 2-3 minutos
-- con la tasa de progresión de cada persona contra su plan estratégico) corre de
-- noche en el portátil con la clave de servicio, que solo habla con la base por la
-- API: no puede lanzar SQL suelto. Esta función es el export que come
-- `agentes/tasa_contra_el_plan.py` (cerebro-alpha-agentes), servido por `rpc`.
--
-- UNA SOLA COPIA DE LA VERDAD, como hizo la `0054` con la mesa del sábado. El cuerpo
-- es la consulta ya probada contra la base real el 12-sep (23 personas) de
-- `tuberia/sql/tasa-contra-el-plan.sql`, copiada byte a byte. Cuando esto esté
-- aplicado, ese archivo pasa a ser `select public.tasa_contra_el_plan_export()`:
-- dos definiciones de la misma verdad es garantizar que diverjan.
--
-- QUIÉN LA PUEDE LLAMAR. Solo `service_role`. Devuelve registro, pesos y perímetros
-- de TODA la cartera: ni un asesorado ni el rol anónimo tienen por qué verla. Se
-- revoca a `public`, `anon` y `authenticated` explícitamente, porque en Postgres una
-- función nueva nace ejecutable por `public` y olvidarlo no da ningún error.
--
-- NO ESCRIBE NADA: `stable` y un único `select`.
-- ===========================================================================
...
per as (
  select a.id, a.nombre,
         ...
         (select jsonb_agg(jsonb_build_object('fecha', md->>'fecha', 'perimetros', md->'perimetros')
                           order by md->>'fecha')
            from public.perfiles p
            cross join lateral jsonb_array_elements(
              case when jsonb_typeof(p.datos->'medidas') = 'array' then p.datos->'medidas' else '[]'::jsonb end) md
           where p.usuario_id = a.id
             and jsonb_typeof(md->'perimetros') = 'object'
             and md->'perimetros' <> '{}'::jsonb) as perimetros,
