-- 0074 · La tasa lee las medidas del cuerpo
--
-- Desde el 2026-09-08 la tarjeta de medidas (`MedidasCard`, catálogo en `domain/medidas.ts`)
-- guarda la cintura y las caderas en `perfiles.datos.medidas[].cuerpo` y escribe `perimetros: {}`.
-- La 0073 solo copiaba `perimetros` y descartaba las tomas con `perimetros` vacío, así que una
-- medida cargada desde la app NUNCA llegaba a la tasa contra el plan: la persona se mide, y su
-- revisión sigue diciendo que no hay medida. Al escribir esta migración había cero tomas en
-- `cuerpo`: no se perdió nada, pero se iba a pedir medidas a tres personas esa misma semana.
--
-- Solo cambia el bloque de perímetros y el conteo `con_perimetros`. Para los datos que ya
-- existían el export sale IDÉNTICO (comprobado contra la 0073 antes de aplicar).
--
-- `create or replace` conserva los privilegios, pero se repiten el revoke y el grant: quien
-- la lea sola tiene que ver que no queda abierta a anon.
...
comment on function public.tasa_contra_el_plan_export() is 'Export de la tasa de progresión contra el plan para la revisión semanal larga. Solo lectura, solo service_role. 0074: lee también las medidas de medidas[].cuerpo (cinturaCm, caderasCm).';
