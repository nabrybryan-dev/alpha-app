-- Comprobación de la migración 0038 (taxonomía por acción articular, fase 1).
-- Las cuatro consultas tienen que devolver CERO filas. Si alguna no lo hace, se
-- restaura desde `respaldo_0038_microciclos` antes de repartir la semana.
...
from microciclos m
join respaldo_0038_microciclos r on r.id = m.id,
...
-- 4 · Todos los microciclos entraron. La 0038 cubre los 73: si alguno se
--     quedó fuera del respaldo, el alcance se calculó mal.
select '4 · microciclo fuera del respaldo' as señal, m.id
from microciclos m
where m.id not in (select id from respaldo_0038_microciclos);
