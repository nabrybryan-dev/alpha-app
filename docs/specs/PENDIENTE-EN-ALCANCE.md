...
**Contra qué árbol se comprobó.** Cada fila se verificó leyendo el código del **árbol
integrado** (`integra/salon`), no el de la rama donde se escribe esto, porque una rama de
capa va siempre por detrás. Ahí se cayó una fila: «que las ocho lleguen a `perfiles` y se
validen allí» **ya está hecha** desde `a78f9ca` —la ficha construye las ocho con las claves
del dominio y las pasa por `revisarMedidas` antes de guardar (`MedidasCard.tsx:147-153` y
`:172`), y guarda en `MedidaCorporal.cuerpo` dejando `perimetros` vacío a propósito
(`:188-189`)—, así que desaparece en vez de quedarse aquí dando una alarma falsa.
...
| **4.** La encuesta pide exactamente las ocho | **Sí, cerrado de punta a punta** | `encuesta-de-medidas.md` (interfaz) y `medidas-en-el-dominio.md` (dominio). El test cuenta los campos por su papel y compara la lista con las ocho **en orden**, así que una novena sale en rojo por las dos mitades. Y desde `a78f9ca` las dos mitades **encajan**: la ficha construye las ocho con las claves del dominio y las pasa por `revisarMedidas` antes de guardar (`MedidasCard.tsx:147-153` y `:172`), así que el rango y la etiqueta no están escritos dos veces. Es el único de los diez puntos que esta tanda cierra. |
| **5.** Al cambiar las medidas cambian las proporciones | **No** | Se piden, se validan y se guardan bien; **nadie las lee** (`cuerpoDelAsesorado.ts:65-73`). Tres filas de la tabla de arriba son este punto. Es el escalón siguiente y el que más cerca está: el dato ya está limpio en `MedidaCorporal.cuerpo`, solo falta que alguien lo consuma. |
...
| **10.** Ningún texto aprobado cambia de sentido sin revisión de Bryan | Ninguna de las seis toca prescripciones | Pero hay **una decisión que Bryan debería ver y no es una prescripción**: `encuesta-de-medidas.md` saca la báscula de «Mis medidas» —desde ahora el peso se anota en el check-in del día—. Está escrita en la spec y no está aprobada. La otra mitad de este apunte —que el historial de perímetros se parte en dos— dejó de ser una nota de texto y es ya un ítem con su fila propia en `FUERA-DE-ALCANCE.md`. |
...
que era la sección que este cambio se lleva. **Comprobado el 7-sep contra el árbol
integrado (`integra/salon`): la referencia sigue colgada allí también** — el comentario
sigue con ese texto y esa spec ya solo tiene §1 a §4. Ese apunte vive ahora en las filas de
esta tabla. Cambiar el comentario es de quien manda en `src/`, y por eso queda dicho aquí y
no hecho.
